...
def create_arguments():
    ...
    optional_args = blast_parser.add_argument_group('optional arguments')
    optional_args.add_argument('--custom_db_file', help='BLAST database of additional proteins', default=None)
    optional_args.add_argument('--custom_taxonomy_file', help='taxonomic assignment of each genomes in custom database',
                              default=None)

    optional_args.add_argument('--homology_search', choices=['blastp-fast', 'blastp', 'diamond'],
                              help="type of homology search to perform", default='blastp-fast')
    optional_args.add_argument('-e', '--evalue', help='evalue cutoff for identifying homologs', type=float, default=1e-5)
    optional_args.add_argument('-p', '--per_identity', help="percent amino acid identity for identifying homologs",
                              type=float, default=30.0)
    optional_args.add_argument('-a', '--per_aln_len',
                              help="percent alignment length of query sequence for identifying homologs", type=float,
                              default=50.0)
    optional_args.add_argument('-m', '--max_matches', help="maximum number of matches per query protein", type=int,
                              default=50000)
    optional_args.add_argument('--restrict_taxon',
                              help='restrict alignment to specific taxonomic group (e.g., d__Archaea)', default=None)

    optional_args.add_argument('--msa_program', choices=['mafft', 'muscle'],
                              help="program to use for multiple sequence alignment (MSA)", default='mafft')

    optional_args.add_argument('--min_per_taxa', help='minimum percentage of taxa required to retain column in MSA',
                              type=float, default=50.0)
    optional_args.add_argument('--consensus',
                              help='minimum percentage of the same amino acid required to retain column in MSA',
                              type=float, default=0.0)
    optional_args.add_argument('--min_per_bp',
                              help='minimum percentage of base pairs in MSA required to keep a trimmed sequence',
                              type=float, default=50.0)
    optional_args.add_argument('--use_trimAl',
                              help='filter columns in MSA using trimAl (other trimming parameters are ignored)',
                              action='store_true')

    optional_args.add_argument('--tree_program', choices=['fasttree', 'raxml'], help="program to use for tree inference",
                              default='fasttree')
    optional_args.add_argument('--prot_model', choices=['LG', 'WAG', 'AUTO'],
                              help='protein substitution model for tree inference', default='LG')

    optional_args.add_argument('--skip_rooting', help="do not perform midpoint rooting before decorating tree",
                              action='store_true')

    optional_args.add_argument('--cpus', type=int, help='CPUs to use throughout the process', default=1)
    optional_args.add_argument('--silent', help="suppress output", action='store_true')
    optional_args.add_argument('-h', '--help', action="help", help="show help message")
    ...
    optional_args = concat_parser.add_argument_group('optional arguments')
    optional_args.add_argument('--split_chars', nargs='?',
                              help='character(s) to use for splitting taxa and gene identifiers', default='~')

    optional_args.add_argument('--min_per_gene', help='minimum percentage of genes required to retain taxa', type=float,
                              default=50.0)
    optional_args.add_argument('--min_per_bps', help='minimum percentage of base pairs required to retain taxa',
                              type=float, default=50.0)
    ...
    optional_args = reduce_parser.add_argument_group('optional arguments')
    optional_args.add_argument('--msa_program', choices=['mafft', 'muscle'],
                              help="program to use for multiple sequence alignment (MSA)", default='mafft')
    optional_args.add_argument('--min_per_taxa', help='minimum percentage of taxa required to retain column in MSA',
                              type=float, default=50.0)
    ...
    optional_args = bootstrap_parser.add_argument_group('optional arguments')
    optional_args.add_argument('-r', '--num_replicates', help='number of bootstrap replicates to perform', type=int,
                              default=100)
    optional_args.add_argument('--tree_program', choices=['fasttree', 'raxml'],
                              help="program to use for inferring bootstrap trees", default='fasttree')
    optional_args.add_argument('--prot_model', choices=['LG', 'WAG'],
    ...
    optional_args = prune_parser.add_argument_group('optional arguments')
    optional_args.add_argument('--silent', help="suppress output", action='store_true')
    optional_args.add_argument('-h', '--help', action="help", help="show help message")
    ...
    optional_args = run_prokka_parser.add_argument_group('optional arguments')
    optional_args.add_argument('-x', '--extension', default='fna',
                              help="extension of genome FASTA files (other files in directory are ignored)")
    optional_args.add_argument('--kingdom', help='kingdom to use for gene annotation', choices=['Archaea', 'Bacteria'],
                              default='Bacteria')
    optional_args.add_argument('--cpus', type=int, default=1, help='number of cpus to use')
    optional_args.add_argument('--silent', help="suppress output", action='store_true')
    optional_args.add_argument('-h', '--help', action="help", help="show help message")
    ...
    optional_args = create_db_parser.add_argument_group('optional arguments')
    optional_args.add_argument('-m', '--max_taxa', type=int, default=50, help='maximum taxa to retain in a named group')
    optional_args.add_argument('-r', '--rank', type=int, default=5,
                              help='rank to preform dereplication [0=domain, 6=species]')
    optional_args.add_argument('-p', '--per_identity', type=float, default=90.0,
                              help="percent identity for subsampling similar genes")
    optional_args.add_argument('-a', '--per_aln_len', type=float, default=90.0,
                              help="percent alignment length for subsampling similar genes")
    optional_args.add_argument('-x', '--extension', default='faa',
                              help="extension of files with called genes (other files in directory are ignored)")
    optional_args.add_argument('--taxonomy', default=None, help='taxonomy string for each genome')
    optional_args.add_argument('--type_strains', default=None,
                              help='file specifying type strains that should not be filtered')
    optional_args.add_argument('--genomes_to_process', default=None,
                              help='list of genomes to retain instead of performing taxon subsampling')
    optional_args.add_argument('--keep_all_genes', action="store_true", default=False,
                              help='restricts filtering to taxa')
    optional_args.add_argument('--no_reformat_gene_ids', action="store_true", default=False,
    ...
    optional_args = arb_db_parser.add_argument_group('optional arguments')
    optional_args.add_argument('--silent', help="suppress output", action='store_true')
    optional_args.add_argument('-h', '--help', action="help", help="show help message")
    ...
    optional_args = robinson_foulds_parser.add_argument_group('optional arguments')
    optional_args.add_argument('--weighted', help="perform weighted Robinson-Foulds", action='store_true')
    optional_args.add_argument('--taxa_list', help='prune trees to specified by taxa before comparison')
    optional_args.add_argument('--silent', help="suppress output", action='store_true')
    optional_args.add_argument('-h', '--help', action="help", help="show help message")
    ...
    optional_args = supported_splits_parser.add_argument_group('optional arguments')
    optional_args.add_argument('--split_file',
                              help='output file with information about congruent and incongruent splits')
    optional_args.add_argument('-s', '--min_support', type=float, default=70, help='minimum support to consider split')
    optional_args.add_argument('-d', '--max_depth', type=float, default=1.0,
    ...
    optional_args = missing_splits_parser.add_argument_group('optional arguments')
    optional_args.add_argument('-s', '--min_support', type=float, default=70,
    ...
    optional_args = midpoint_parser.add_argument_group('optional arguments')
    optional_args.add_argument('--silent', help="suppress output", action='store_true')
    optional_args.add_argument('-h', '--help', action="help", help="show help message")
    return parser
